...
Assert-Contains 'CreateToggleRow\(FarmTab,\s*"Auto Rejoin"' 'Missing Auto Rejoin toggle'
Assert-NotContains 'local\s+function\s+BuildV6Menu\(' 'V6 menu builder must not consume top-level local register'
Assert-Contains 'function\s+_G\.BugonBuildV6Menu\(' 'V6 menu must use global builder to avoid Luau top-level local register limit'
Assert-Contains 'pcall\(_G\.BugonBuildV6Menu\)' 'V6 menu spawn must call global builder'
...
Assert-Contains 'ForgeTab' 'Missing Forge main tab'
Assert-Contains 'ForgeTabButton' 'Missing Forge main navigation button'
Assert-Contains 'ForgeCraftPage' 'Missing Forge craft subview'
Assert-Contains 'ForgeTargetsPage' 'Missing Forge targets subview'
Assert-Contains 'CreateButton\(ForgeNavigation,\s*"CRAFT"\)' 'Missing CRAFT Forge sub-tab'
Assert-Contains 'CreateButton\(ForgeNavigation,\s*"TARGETS"\)' 'Missing TARGETS Forge sub-tab'
Assert-Contains 'FarmTabButton\.Size\s*=\s*UDim2\.new\(1\s*/\s*3' 'Main tabs must use equal thirds'
Assert-Contains 'UtilityTabButton\.Position\s*=\s*UDim2\.new\(1\s*/\s*3' 'Utility main tab must occupy middle third'
Assert-Contains 'ForgeTabButton\.Position\s*=\s*UDim2\.new\(2\s*/\s*3' 'Forge main tab must occupy final third'
Assert-NotContains 'AutoForgeTabButton' 'Forge must not remain under Utility navigation'
...
Assert-Contains 'CreateSelectorDropdown\(Parent,\s*Name,\s*PositionY,\s*OpenUpwards\)' 'Selector dropdown helper must support upward popup placement'
Assert-Contains 'Options\.AnchorPoint\s*=\s*OpenUpwards\s+and\s+Vector2\.new\(0,\s*1\)\s+or\s+Vector2\.new\(0,\s*0\)' 'Upward selector popup must anchor from its bottom edge'
Assert-Contains 'CreateSelectorDropdown\(DungeonPage,\s*"Difficulty",\s*126,\s*true\)' 'Difficulty dropdown must open upward inside clipped panel'
...
Assert-Contains '\(_G\.AutoSell\s+or\s+Config\.RecoveryPending\)\s+and\s+IsInLobby\s+and\s+IsInLobby\(\)' 'AutoSell must remain lobby-only'
Assert-Contains 'local\s+OreStats\s*=\s*\{\s*Current\s*=\s*0\s*,\s*Max\s*=\s*0\s*,\s*RejoinStatus\s*=\s*""\s*\}' 'Missing farm stats cache'
Assert-Contains 'ORE:\s*"\s*\.\.\s*tostring\(OreStats\.Current\)\s*\.\.\s*"/"\s*\.\.\s*tostring\(OreStats\.Max\)' 'Stats label must show ORE current/max'
Assert-Contains 'REJOIN:\s*"\s*\.\.\s*tostring\(RejoinWatchdog\.Status\)' 'Stats label must show rejoin status'
...
Assert-Contains 'StatsLabel\.Size\s*=\s*UDim2\.new\(1,\s*0,\s*0,\s*96\)' 'V6 stats label must fit four lines'
