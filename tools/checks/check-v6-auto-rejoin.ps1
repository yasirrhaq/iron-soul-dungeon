...
Assert-Contains 'return\s+_G\.AutoRejoin\s+and\s+RejoinWatchdog\.RecoveryActive' 'HardStuck must not block normal automation'
Assert-Contains 'Config\.RecoveryPending\s*=\s*false' 'HardStuck and stale recovery must clear pending state'
Assert-Contains 'RECOVERY_PENDING_CLEARED_ACTIVE_SESSION' 'Stale pending recovery must clear instead of forcing teleport'
Assert-NotContains 'PENDING_NOT_LOBBY' 'Stale recovery must not force restricted lobby teleport'
