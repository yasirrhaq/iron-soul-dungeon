...
Assert-Contains 'WeaponScythe\s*=\s*\{[^}]*Category\s*=\s*"Weapon"[^}]*OreCount\s*=\s*20[^}]*Chance\s*=\s*90' 'Missing Scythe recipe'
Assert-Contains '"WeaponFistCommon",\s*"WeaponBow",\s*"WeaponBowRelic",\s*"WeaponScythe",\s*"WeaponFistLuxury"' 'Bow and Scythe recipes must be selectable before luxury relic'
