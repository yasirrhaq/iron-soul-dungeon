...
Assert-Contains 'WeaponBow\s*=\s*\{[^}]*Category\s*=\s*"Weapon"[^}]*OreCount\s*=\s*18[^}]*RelicId\s*=\s*"BowRelic_1"' 'Missing Bow recipe'
Assert-Contains '"WeaponFistCommon",\s*"WeaponBow",\s*"WeaponFistLuxury"' 'Bow recipe must be selectable before luxury relic'
