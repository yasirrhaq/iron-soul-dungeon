...
Assert-Contains 'function\s+AutoForge\.WaitForResult\(' 'Missing result replication wait'
Assert-Contains 'WindowUtil:Open\("ScreenForgeResult"\)' 'Auto Forge must open native result screen'
Assert-Contains 'function\s+AutoForge\.WaitForResultClear\(' 'Batch must wait for Accept or Delete'
Assert-NotContains 'InvokeServer\("ForgeResult",\s*true\)' 'Auto Forge must not accept result before user sees it'
