...
Assert-Contains 'function\s+AutoSkillVisualSuppression\.SuppressPlayingTracks\(' 'Missing per-frame animation suppression'
Assert-Contains 'RunService\.RenderStepped:Connect' 'Auto-skill animation weight must be enforced every frame'
Assert-Contains 'Enum\.AnimationPriority\.Action' 'Unnamed action-priority skill tracks must be suppressed'
...
Assert-Contains 'PointLight' 'Missing PointLight suppression'
Assert-Contains 'SpotLight' 'Missing SpotLight suppression'
Assert-Contains 'SurfaceLight' 'Missing SurfaceLight suppression'
Assert-Contains 'Decal' 'Missing Decal suppression'
Assert-Contains 'Texture' 'Missing Texture suppression'
Assert-Contains 'LocalTransparencyModifier\s*=\s*1' 'Mesh and part VFX must be locally hidden'
Assert-Contains 'function\s+AutoSkillVisualSuppression\.ScanEffectRoots\(' 'Pooled VFX must be rescanned during auto casts'
