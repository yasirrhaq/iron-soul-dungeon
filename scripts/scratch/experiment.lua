...
local function CanEnterPortal(currentTime)
    return (currentTime - LastEnemySeen) >= 4.0 and (currentTime - LastBreakableSeen) >= BreakablePortalDelay
end
...
local function IsInLobby()
    ...
    for i = 1, #Children do
        local Name = string.lower(Children[i].Name)
...
RunService.Heartbeat:Connect(function()
    if _G.AutoFarm and _G.UndergroundMode and not IsInLobby() and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
