...
-- =========================================================================
-- [SMART PRECISE] AUTO WAVE TRIGGER & STAGE PORTAL PROGRESSION
-- =========================================================================
...
local function TeleportToNextStagePortal()
    ...
    if WaveSpawnTouch and (os.clock() - LastWaveTriggerAttempt) >= 4.0 then
        ...
        -- Jika mendeteksi ada Round yang siap di-trigger (seperti Round2 milikmu saat ini)
        if ActiveTouchZone then
            print("🌊 [SmartTrigger] Mendeteksi Wave Aktif: " .. ActiveTouchZone.Name .. " (TouchInterest Terdeteksi!)")
            LastWaveTriggerAttempt = os.clock()
            
            -- Teleport paksa ke posisi sensor agar server langsung memunculkan musuh
            MyRoot.CFrame = CFrame.new(ActiveTouchZone.Position + Vector3.new(0, 2, 0))
            MyRoot.Velocity = Vector3.new(0, 0, 0)
            
            -- Kirim sinyal touch instan lewat API Executor Delta kamu
            ...
            task.wait(1.2) -- Jeda singkat memberi waktu server melakukan spawning musuh
            return -- Keluar dari fungsi agar loop utama beralih fokus membantai musuh baru
        end
    end

    -- STEP 2: JIKA TIDAK ADA SENSOR WAVE YANG MEMILIKI TOUCHINTEREST, BARU TELEPORT PORTAL
    ...
    if BestPortalPart and HighestScore >= 3 then
        ...
        PortalCooldown = true 
        print("🚪 [Portal] Semua wave bersih total. Teleporting ke portal berikutnya: " .. BestPortalPart.Name)
