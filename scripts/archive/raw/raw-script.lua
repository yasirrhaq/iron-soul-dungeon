-- ====================================================================
-- IRON SOUL - ULTIMATE REWRITTEN FARM (V14 SMART DISTANCE GUARD + ANTI-FALL)
-- ====================================================================
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") or obj:IsA("MeshPart") then
            ...
            if DistanceToPortal <= MaxPortalDistance then
                ...
                if string.find(LowerName, "portal") or string.find(LowerName, "gate") or string.find(LowerName, "door") then
                ...
                if obj:FindFirstChildOfClass("TouchTransmitter") then CurrentScore = CurrentScore + 3 end
                if obj.Material == Enum.Material.Neon then CurrentScore = CurrentScore + 3 end
                if obj.Size.Y > 5 and obj.Size.X > 5 then CurrentScore = CurrentScore + 2 end
        ...
        if _G.AutoFarm then
            ...
            if Success and NewTarget then
                ...
                if _G.AutoProgressStage and not IsEnteringPortal and not PortalCooldown then
                    local CurrentTime = os.clock()
                    if (CurrentTime - LastPortalCheck) >= 2.0 then 
