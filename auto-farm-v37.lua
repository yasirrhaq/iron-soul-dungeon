-- ====================================================================
-- IRON SOUL - V37 MASTER EDITION (STRICT REPLAY FILTER & LOBBY SAFE)
-- ====================================================================
...
-- SUPER AGGRESSIVE TARGET SCANNER
...
-- [FIXED V37] STRICT REPLAY DETECTOR FILTER (ANTI-CLICK SOCIAL APPLICATION)
local function ScanAndExecuteReplay()
    ...
    for i = 1, #SemuaGui do
        ...
        if obj:IsA("TextLabel") then
            ...
            -- Gunakan pencarian ganda spesifik "play" DAN "again" untuk menghindari jebakan kata "application"
        ...
        elseif obj:IsA("TextButton") or obj:IsA("ImageButton") then
            local nameLower = string.lower(obj.Name)
            -- Validasi tombol eksklusif tombol dungeon akhir
    ...
    if ReplayButton then
        ...
        task.spawn(function() task.wait(5.0) ServerLoadingLock = false end)
    end
end

-- DYNAMIC FARM MAIN BUFFER LOOP
task.spawn(function()
    while true do
...
-- CORE MOVEMENT LOOP (FLY / UNDERGROUND)
RunService.Heartbeat:Connect(function(dt)
    local Character = LocalPlayer.Character
    local MyRoot = Character and Character:FindFirstChild("HumanoidRootPart")
    
    if IsInLobby() or ServerLoadingLock or not _G.AutoFarm or not MyRoot or IsEnteringPortal then return end
    ...
    -- JIKA MELEBIHI 25 STUDS LANGIT: Aktifkan mode jepretan hitbox kilat dinamis
    if _G.TinggiMelayang > 25 and not IsExtractingEgg then
        local PosisiSerangKilat = CFrame.new(TargetPos.X + OffsetX, TargetPos.Y + 3, TargetPos.Z + OffsetZ)
        
        if _G.UndergroundMode then MyRoot.CFrame = PosisiSerangKilat * CFrame.Angles(math.rad(90), 0, 0)
        else MyRoot.CFrame = PosisiSerangKilat * CFrame.Angles(math.rad(-90), 0, 0) end
        ...
        if _G.UndergroundMode then MyRoot.CFrame = CFrame.new(FinalPosition) * CFrame.Angles(math.rad(90), 0, 0)
        else MyRoot.CFrame = CFrame.new(FinalPosition) * CFrame.Angles(math.rad(-90), 0, 0) end
    else
        if _G.UndergroundMode then MyRoot.CFrame = CFrame.new(FinalPosition) * CFrame.Angles(math.rad(90), 0, 0)
        else MyRoot.CFrame = CFrame.new(FinalPosition) * CFrame.Angles(math.rad(-90), 0, 0) end
...
TitleLabel.Text = "IRON SOUL V37 REPLAY-FIX"
...
UserInputService.InputChanged:Connect(function(input)
    if ActiveSlider and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        if ActiveSlider == "HEIGHT" then
            ...
            _G.TinggiMelayang = Val Config.TinggiMelayang = Val
